...
const ROM_URL = "roms/tetris.nes";
const DEBUG = true;
const STARTING_X = 3;
let WELL_HEIGHT = 20;
let WELL_WIDTH = 10;
const WELL_HEIGHT_CALCULATION_MARGIN = 5;
...
onmessage = e => {
    let best_position = calculate_best_position(e.data[0], e.data[1], e.data[2]);

    console.log("lines", best_position.weights.pure_lines);
    console.log("pos", best_position);
    // @ts-ignore
    postMessage(best_position);
};

function calculate_best_position(
    ...
    for (let schema_index = 0; schema_index < rotation_schemas.length; schema_index++) {
        for (let next_schema = 0; next_schema < next_rotation_schemas.length; next_schema++) {
            for (let row = height; row <= WELL_HEIGHT; row++) {
                for (let cell = -2; cell <= WELL_WIDTH + 1; cell++) {
...
function calculate_for_position(
...
function calculate_points(well: Array<Array<number>>) {
    let heights = [0];
    let lines = 0;
    let holes = 0;
    let bumpiness = 0;
    ...
    // Removing lines here gives better effects
